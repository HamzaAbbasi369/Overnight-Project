...
$need167Tabs = ($rx_strength >= 0) && ($rx_strength <= 6.1);
$need174Tabs = ($rx_strength >= 6.2) && ($rx_strength <= 8.2);
...
ORDER BY package_name',
        ...
        $sql_tabs='CASE TRUE WHEN (material = "1.67 Super Thin Lenses Package") THEN "middle" WHEN (material = "Trivex HD Lenses Package") THEN "last" ELSE "first" END AS tabs';
...
END AS tabs';
	...
	$materialSQL = 'material IN("1.67 Super Thin Lenses Package", "1.74 Ultra Thin Lenses Package")';

	$sql_tabs='CASE TRUE
WHEN (material = "1.67 Super Thin Lenses Package") THEN "first"
WHEN (material = "1.74 Super Thin Lenses Package") THEN "first"
ELSE "first"
END AS tabs';
        $sql = sprintf('SELECT package_id, package_name, package_description, package_option_name, material,
    b.anti_glare coating,b.easy_clean, tint, tint_options, price, sale_price, b.uv_protection, b.scratch_protection, recommend,
...
$sql_add = "SELECT package_id, package_name, package_description, package_option_name, material,
    ...
    ." FROM rx_packages a, rx_package_options b
    WHERE a.package_option_id=b.package_option_id AND a.package_option_id=99 $rxtypeSQL $rx_progressive_typeSQL $premiumSQL $use_frameSQL
    AND material IN ('$materials','Polycarbonate Lenses Package') $min_maxSQL UNION SELECT package_id, package_name, package_description, package_option_name, material, b.anti_glare coating,b.easy_clean, tint, tint_options, price, sale_price, b.uv_protection, b.scratch_protection, recommend, $sql_tabs , a.package_option_id FROM rx_packages a, rx_package_options b WHERE a.package_option_id=b.package_option_id AND a.package_option_id=100 $rxtypeSQL $min_maxSQL $use_frameSQL ORDER BY PACKAGE_OPTION_ID, package_name ASC";
...
function drawPackages($usage_fee, $impact_fee, $pfactor, $rxtype, $result, $pricetoremove, $tab_key = null)
{
	...
	$i++;
        ...
        //echo "<br/>PACKAGE PRICE CALC : ".$res['price']." - USAGE FEE: ".$usage_fee." - IMPACT_FEE: ".$impact_fee." - PRICE TO REMOVE: ".$pricetoremove. " = ".$package_price;
	$sale_price = (!empty($res['sale_price']) ? $res['sale_price'] - $usage_fee - $impact_fee - $pricetoremove: null);
	//echo "<br/>SALE PRICE CALC: ".$res['sale_price']."- USAGE FEE: ".$usage_fee." - IMPACT_FEE: ".$impact_fee." - PRICE TO REMOVE: ".$pricetoremove." = ".$sale_price;
	$html = "";
...
<?php if ($result) : ?>
    ...
    <?php elseif ($need167Tabs) : ?>
 <?php 
        
        if($_REQUEST['action'] == 'rx_get_preset_packages'){
            echo "<h1>Select A Lens Package</h1>";
        }
...
<div class="clearfix">
    <div class="large-12 columns text-center customize-package clear-padding">
        ...
        <button class="btn btn-warning footer switch_std_special">VIEW ADDITIONAL PACKAGES</button>
    </div>
</div>
...
<script type="text/javascript">
	...
	$('.switch_std_special').css('cssText', 'display: none');
    } else {
	$('.switch_std_special').show();
	//$('.switch_std_special').css('cssText', 'color: white !important;');
    }
	...
	$('.switch_std_special').addClass('special_pkg_b_safari');
    	console.log('add class special_pkg_b_safari');
    } else {
    ...
    $('.switch_std_special').on('click', function() {
	    ...
	    if ( /^((?!chrome|android).)*safari/i.test(navigator.userAgent)) {
		$('.switch_std_special').removeClass('special_pkg_b_safari');
	    } else {
	    ...
	    $('.rx-product-info-package').show();
            ...
            $('.switch_std_special').html('VIEW ADDITIONAL PACKAGES');
        }
    });
