...
class WcRxInstall
{
    ...
    public static function init()
    {
        ...
        add_action('in_plugin_update_message-woocommerce/woocommerce.php', [__CLASS__, 'in_plugin_update_message']);
        add_filter('plugin_action_links_' . WC_RX_PLUGIN_BASENAME, [__CLASS__, 'plugin_action_links']);
        add_filter('plugin_row_meta', [__CLASS__, 'plugin_row_meta'], 10, 2);
        add_filter('wpmu_drop_tables', [__CLASS__, 'wpmu_drop_tables']);
        add_filter('cron_schedules', [__CLASS__, 'cron_schedules']);
        add_action('woocommerce_plugin_background_installer', [__CLASS__, 'background_installer'], 10, 2);
    }
    ...
    {
        if (!defined('IFRAME_REQUEST') && get_option('woocommerce_rx_version') !== WcRx()->version) {
            self::install();
            do_action('woocommerce_rx_updated');
        }
    }
    ...
    public static function install()
    {
        ...
        if (!defined('WC_RX_INSTALLING')) {
            define('WC_RX_INSTALLING', true);
        }
...
//
        ...
        $current_rx_version = get_option('woocommerce_rx_version', null);
        $current_db_version = get_option('woocommerce_rx_db_version', null);

        WC_Admin_Notices::remove_all_notices();

        // No versions? This is a new install :)
        if (is_null($current_rx_version) && is_null($current_db_version) && apply_filters('woocommerce_enable_setup_wizard', true)) {
     ...
     *        woocommerce_order_items - Order line items are stored in a table to make them easily queryable for
     *        reports
     *        woocommerce_order_itemmeta - Order line item meta is stored in a table for storing extra data.
     *        woocommerce_tax_rates - Tax Rates are stored inside 2 tables making tax queries simple and efficient.
     *        woocommerce_tax_rate_locations - Each rate can be applied to more than one postcode/city hence the second
     *        table.
     */
...
CREATE TABLE {$wpdb->prefix}woocommerce_termmeta (
  ...
  KEY meta_key (meta_key($max_index_length))
    ...
    {
        delete_option('woocommerce_rx_db_version');
        add_option('woocommerce_rx_db_version', is_null($version) ? WcRx()->version : $version);
    }
    ...
    {
        if (!empty($_GET['do_update_woocommerce_rx'])) {
            self::update();
            WC_Admin_Notices::add_notice('update');
        }
        if (!empty($_GET['force_update_woocommerce_rx'])) {
            do_action('wp_wc_rx_updater_cron');
            wp_safe_redirect(admin_url('admin.php?page=wc-rx-settings'));
        }
    }
    ...
    public static function create_pages()
    {
        ...
        $pages = apply_filters('woocommerce_create_pages', [
            'shop'      => [
    ...
    private static function get_core_capabilities()
    {
        ...
        $capability_types = ['product', 'shop_order', 'shop_coupon', 'shop_webhook'];
    ...
    public static function background_installer($plugin_to_install_id, $plugin_to_install)
    {
        if (!empty($plugin_to_install['repo-slug'])) {
            ...
            if (!$installed) {
                ...
                try {
                    $plugin_information = plugins_api('plugin_information', [
