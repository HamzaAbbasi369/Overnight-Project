...
class Ong_Array_Helper
{
     ...
     * @return array
     */
    public static function unflatternArray($arr)
    {
        $unflatten = [];
        ...
        foreach ($arr as $key => $value) {
            ...
            $first_key = self::getNormalizedArrayKey($first_key);
            if (sizeof($key_list) > 0) {
                $remaining_keys = implode('.', $key_list);
                $subarray       = self::unflatternArray([$remaining_keys => $value]);

                foreach ($subarray as $sub_key => $sub_value) {
                    $sub_key = self::getNormalizedArrayKey($sub_key);
                    if (isset($unflatten[$first_key][$sub_key])) {
                        $unflatten[$first_key][$sub_key] =
                            self::arrayMergeRecursiveDistinct($unflatten[$first_key][$sub_key], $sub_value);
                    } else {
     ...
     * @return string|int
     */
    public static function getNormalizedArrayKey($key)
    {
        if (is_numeric($key)) {
            $key = absint($key);
        }
