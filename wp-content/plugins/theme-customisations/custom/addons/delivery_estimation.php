...
function get_production_days($rx_type, $late_day, $rimless, $second_day_delivery, $fl_41)
{
...
function get_estimated_delivery($order)
{
    ...
    foreach ($order->get_items() as $item_id => $item_data) {
        ...
        if ($item_data['_all_lens_data']['tint'] == 'FL-41 Tint') {
            $fl_41 = 1;
        }
        ...
        if ($item_data['_all_lens_data']['rush_3day'] == 1) {
            $rush_3day = 1;
        }
    ...
    $rush = isset($_GET['rush']) ? $_GET['rush'] : $rush;
    */
    ...
    $t = $mon."-".$today_tmp['tm_mday']."-".date("y");
    ...
    if ($single_vision == 1) {
        if ($rush == 1) {
            if ($today_day == "Fri" && $late_day == 0) {
                ...
                if ($rimless == 0 && $fl_41 == 0) {
                    ...
                    $production_days = "Ships Tuesday";
                    $shipping_days = 1;
                    $deliver_by = addDays($today, 4 + $shipping_days, "", $holidays);
                } elseif ($rimless == 1 && $fl_41 == 0) {
                    $production_days = 4;
                    $shipping_days = 2;
                    ...
                    $production_days = 5;
                    $shipping_days = 2;
                    $deliver_by = addDays($today, 5 + 2, "", $holidays);
                }
                ...
                if ($rimless == 0 && $fl_41 == 0) {
                    ...
                    $shipping_days = 1;
                    $deliver_by = addDays($today, 3, array("Sunday"), $holidays);
                } elseif ($rimless == 1 && $fl_41 == 0) {
                    $production_days = 3;
                    $shipping_days = 2;
	    ...
	    if ($late_day == 1) {
            ...
            if ($shipping_name == "UPS 2nd Day Air (2 Days)" || $shipping_name == "UPS 2nd Day Air") {
                $production_days_data = get_production_days("single_vision", $late_day, $rimless, 1, $fl_41);
                $production_days = $production_days_data[0];
                $shipping_days = $production_days_data[1];
                $deliver_by = "";
            } else {
                $production_days_data = get_production_days("single_vision", $late_day, $rimless, 0, $fl_41);
                $production_days = $production_days_data[0];
                $shipping_days = $production_days_data[1];
                $deliver_by = "";
            }
        ...
        if ($rush == 1) {
            if ($today_day == "Fri" && $late_day == 0) {
                if ($rimless == 0 && $fl_41 == 0) {
                    ...
                    $shipping_days = 1;
                    $deliver_by = addDays($today, 6, "", $holidays);#strftime((strtotime("+1 day", $deliver_by)), "%m/%d/%Y");
                } elseif ($rimless == 1 && $fl_41 == 0) {
                    $production_days = 5;
                    $shipping_days = 2;
                    $deliver_by = addDays($today, ($production_days + $shipping_days), "", $holidays);
                ...
                if ($rimless == 0 && $fl_41 == 0) {
                    $production_days = "5 (Ships on Tuesday)";
                    $shipping_days = 1;
                    $deliver_by = addDays($today, (5 + $shipping_days) - 1, "", $holidays);
                } elseif ($rimless == 0 && $fl_41 == 1) {
                    $production_days = "6 (Ships on Wednesday)";
                    $shipping_days = 1;
                    $deliver_by = addDays($today, (6 + $shipping_days) - 1, "", $holidays);
                } elseif ($rimless == 1 && $fl_41 == 0) {
                    $production_days = 6;
                    $shipping_days = 3;
                ...
                if ($rimless == 0 && $fl_41 == 0) {
                    ...
                    $production_days = "5 (Ships on Wednesday)";
                    $shipping_days = 1;
                    $deliver_by = addDays($today, 5, "", $holidays);
                ...
                if ($rimless == 0 && $fl_41 == 0) {
                    ...
                    $production_days = "4 (Ships on Wednesday)";
                    $shipping_days = 1;
                    $deliver_by = addDays($today, 4, "", $holidays);
                } elseif ($rimless == 1 && $fl_41 == 0) {
                    $production_days = 4;
            ...
            if ($shipping_name == "UPS 2nd Day Air (2 Days)" || $shipping_name == "UPS 2nd Day Air") {
                $production_days_data = get_production_days("progressive_bifocal", $late_day, $rimless, 1, $fl_41);
                $production_days = $production_days_data[0];
                $shipping_days = $production_days_data[1];
                $deliver_by = "";
            } else {
                $production_days_data = get_production_days("progressive_bifocal", $late_day, $rimless, 0, $fl_41);
                $production_days = $production_days_data[0];
                $shipping_days = $production_days_data[1];
                $deliver_by = "";
            }
